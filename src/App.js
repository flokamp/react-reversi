...
class App extends Component {
	...
	checkMove(i, j, apply, black_turn) {
		...
		let our =
			black_turn === undefined ? ~~!this.state.black_turn : ~~!black_turn;
		let other = ~~!our;
	...
	handleClick(i, j) {
		...
		if (moved) {
			...
			for (let i = 0; i < 8; i++) {
				...
				if (hasMove) {
					console.log(
						"found possible move for",
						black_turn ? "white" : "black"
					);
			...
			if (!hasMove) {
				console.log(
					"current player has no moves! passing turn to other player"
				);
				//this.setState({black_turn: !black_turn});
				console.log(
					"checking possible move for",
					black_turn ? "white" : "black"
				);
				for (let i = 0; i < 8; i++) {
					for (let j = 0; j < 8; j++) {
		...
		return (
			<div className="page">
				...
				<div className="boardContainer">
					<div className="board">
						<Row
							over={this.state.gameOver}
							key={0}
							i={0}
							handleClick={this.handleClick}
							array={this.state.array[0]}
						/>
						<Row
							over={this.state.gameOver}
							key={1}
